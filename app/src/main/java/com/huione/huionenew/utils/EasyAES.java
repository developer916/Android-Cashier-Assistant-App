...
public class EasyAES {
    ...
    //教授加密算法测试key
    private static String testKey = "m@t1OSXOqIGrb4H!";
    ...
    public static String encryptString(String content) {
        if (isTest) {
            ...
            byte[] entryBytes = encodeAes256(content.getBytes(), testKey.getBytes());
            return new String(entryBytes);
        } else {
            //这里填写密码和iv字符串，注意要确保16位的
            EasyAES ea = new EasyAES("!^!z$L77JRQ9w%Q#", 128, "1234567812345678");
    ...
    public static String decryptString(String content) {
        if (isTest) {
            ...
            byte[] decodeBytes = decodeAes256(content.getBytes(), testKey.getBytes());
    ...
    public static native byte[] encodeAes256(byte[] recv_buf, byte[] key_buf);
//	private static String testKey = "m@t1OSXOqIGrb4H!";

    public static native byte[] decodeAes256(byte[] recv_buf, byte[] key_buf);
